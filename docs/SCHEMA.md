...
## **2. Table: `language_levels`**

Lookup table — defines all possible levels per language.

```sql
create table public.language_levels (
  id serial primary key,
  language_code text not null, -- 'zh-TW', 'ja', 'ko', 'vi', 'th', 'de', 'zh-CN'
  level_code text not null,     -- 'TOCFL_3', 'JLPT_N5', 'A2'
  cefr_equivalent text,         -- 'A1', 'B1', 'C1' (for cross-language mapping)
  display_name text not null,   -- 'TOCFL Level 3', 'JLPT N5', 'A2'
  description text,
  sort_order int not null,      -- 1-9 for progression
...
## **3. Table: `user_language_profiles`**

Links users to languages they're learning, with current level.
...
## **4. Table: `curriculum_words`**

Words bucketed by language + level.
...
create table public.curriculum_words (
  ...
  word text not null,
  pinyin_romaji text,          -- optional phonetic
  definition text not null,
  example_sentence text,
  part_of_speech text,
  difficulty_score numeric(3,1), -- 1.0 to 10.0
...
Words users manually save/highlight.
...
create table public.saved_words (
  ...
  context_snippet text,          -- where they found it
...
Scheduled words for the notification system.
...
## **8. Supabase Auth Trigger (Auto-create Profile)**
...
profiles (id = auth.users.id)
    │
    ├── user_language_profiles (user_id → profiles.id)
    │       │
    │       └── language_levels (current_level_id → language_levels.id)
    │
    ├── saved_words (user_id → profiles.id)
    │       │
    │       └── curriculum_words (word_id → curriculum_words.id)
    │               │
    │               └── language_levels (level_id → language_levels.id)
    │
    └── daily_words (user_id → profiles.id)
            │
            └── curriculum_words (word_id → curriculum_words.id)
```
